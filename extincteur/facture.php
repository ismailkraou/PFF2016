...
session_start();
	 ...
	 if(!isset($_SESSION['login']) or $_SESSION['login']=='' or !isset($_GET['id']) ){
		...
		$id=$_GET['id'];
		$reqCmd="select * from factur where id = $id";
		$rsCmd=mysql_query($reqCmd) or die(mysql_error());
		$facture=mysql_fetch_assoc($rsCmd);
		
		$reqDetail="select * from detail_cmd where NumF=$id";
...
<script language="javascript">
	function hello(){
		document.getElementById("zack").innerHTML = "Paragraph changed!";
			  alert("sdfsd");
