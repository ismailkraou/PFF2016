...
session_start();
	 ...
	 if(!isset($_SESSION['login']) or $_SESSION['login']==''){
		...
		$prod1=mysql_fetch_assoc($rsz);
			...
			$reqDetail44="select * from detail_cmd where etat='0' and Id_Prod='$idprod' and type='$type'";
	    ...
	    if($Qte<=$prod1['qteDispo']){
		     if(isset($prod['Id_Prod'])){
			  $Qte+=$prod['Qte'];
			  $idpr=$prod['Id_Prod'];
			             $reqDetail="update detail_cmd set Qte='$Qte' where etat='0' and Id_Prod='$idpr'and type='$type' ";// 
			$rsz1=mysql_query($reqDetail) or die(mysql_error());
			
			
		           }else{
					   
					   		$reqDetail="insert into detail_cmd values('','0','$idprod','$Qte','	$price','0','$type')";
			$rsz1=mysql_query($reqDetail) or die(mysql_error());
...
FROM  `detail_cmd` where etat='0'
GROUP BY  `NumF`  ";
       	$rstotal=mysql_query($total) or die(mysql_error());
		$tot=mysql_fetch_assoc($rstotal);
		$toot=$tot['total'];
			$suball="insert into factur values('','$d','$idcl','	$toot') ";
			mysql_query($suball);
			
			$reqNumCmd="select * from factur  where date='$d' and idCl='$idcl' ORDER BY id DESC LIMIT 1";
			$rsreqNumCmd=mysql_query($reqNumCmd) or die(mysql_error());
			$reqNumfact=mysql_fetch_assoc($rsreqNumCmd);
			$numF=$reqNumfact['id'];
			$reqUpdateDetail="update detail_cmd set NumF='$numF',etat='1' where etat='0'";
			mysql_query($reqUpdateDetail) or die(mysql_error());
			unset( $_SESSION['client']);
				   unset( $_SESSION['date']);
			header("location:facture.php?id=$numF");
			...
			$rs3=mysql_query($req3) or die(mysql_error());
				...
				$req4="select * from detail_cmd where etat='0' ";
  ...
  <div class="container">
    ...
    <div id="main_container">
	  <div class="box paint color_0">
            <div class="title">
              <h4> <i class="icon-random"></i> <span>Commande</span> </h4>
            </div>
            <div class="content ">
		...
		$desi=mysql_fetch_assoc($rsz12);
				...
				?>
                 <tr>
                  ...
                  <td class="ms"><div class="btn-group">  <a class="btn  btn-small" rel="tooltip" data-placement="bottom" href="DeleteDetailCommande.php?id=<?php echo $detail['id']  ?>" data-original-title="Remove"><i class="gicon-remove "></i></a> </div></td>
				   </tr>
				<?php } ?>               
